...
class IssueBallUseCase implements IssueBallUseCaseInputPort {
  final IssueBallTypeRepository _issueBallTypeRepository;

  IssueBallUseCase({@required IssueBallTypeRepository issueBallTypeRepository})
      : _issueBallTypeRepository = issueBallTypeRepository;

  @override
  Future<int> ballHit(
      {@required FBallReqDto reqDto,
      @required IssueBallUseCaseOutputPort outputPort}) async {
    if (outputPort != null) {
  ...
  Future<List<BallImageItemDto>> ballImageListUpLoadAndFillUrls(
    ...
    var fBallImageUploadResDto =
        await _issueBallTypeRepository.ballImageUpload(images: images);
  ...
  Future<FBallResDto> insertBall(
      {@required IssueBallInsertReqDto reqDto,
      @required IssueBallUseCaseOutputPort outputPort}) async {
  ...
  Future<int> updateBall(
      {@required IssueBallUpdateReqDto reqDto,
      @required IssueBallUseCaseOutputPort outputPort}) async {
