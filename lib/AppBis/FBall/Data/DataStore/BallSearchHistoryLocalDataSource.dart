...
abstract class BallSearchHistoryLocalDataSource {
  void saveHistory(
      {required BallSearchBarHistoryDto saveReq,
  ...
  List<BallSearchBarHistory> loadHistory(
      {required SharedPreferences sharedPreferences});
}

@LazySingleton(as: BallSearchHistoryLocalDataSource)
class BallSearchHistoryLocalDataSourceImpl
    implements BallSearchHistoryLocalDataSource {
  ...
  void saveHistory(
      {required BallSearchBarHistoryDto? saveReq,
      required SharedPreferences? sharedPreferences}) {
    List<BallSearchBarHistory> historyList =
        getBallSearchHistoryFromPrefs(sharedPreferences);
    ...
    sharedPreferences?.setString(
        shareBallSearchBarHistory, json.encode(historyList));
  }
  ...
  void insertHistoryFirst(List<BallSearchBarHistory> historyList,
          BallSearchBarHistoryDto? saveReq) =>
      historyList.insert(
          0, BallSearchBarHistory.fromBallSearchBarHistoryDto(saveReq));

  List<BallSearchBarHistory> getBallSearchHistoryFromPrefs(
      SharedPreferences? prefs) {
    List<BallSearchBarHistory> historyList = <BallSearchBarHistory>[];
    if (hasSaveFile(prefs)) {
      String? saveJsonText = getSaveJsonText(prefs);
      ...
      historyList = <BallSearchBarHistory>[];
    }
    return historyList;
  }

  String? getSaveJsonText(SharedPreferences? prefs) =>
      prefs?.getString(shareBallSearchBarHistory);
  ...
  bool hasHistory(BallSearchBarHistoryDto? saveReq,
      List<BallSearchBarHistory> historyList) {
    var indexWhere = historyList
        .indexWhere((element) => element.searchText == saveReq?.searchText);
    if (indexWhere >= 0) {
      return true;
    } else {
      return false;
