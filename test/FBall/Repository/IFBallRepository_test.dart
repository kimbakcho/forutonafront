...
void main() {
  ...
  test('should be result type FBallListUpWrap', () async {
    ...
    //act
    var reslut = await ifBallRepository.listUpFromInfluencePower(
        listUpReqDto: searchCondition);
    //assert
    verify(mockIFBallRemoteDataSource.listUpFromInfluencePower(
        anyNamed('fBallListUpReqDto'), anyNamed('fDio')));
  ...
  test('should be call reqBalls Type Match', () async {
    ...
    setDateSourceListUpBall(mockIFBallRemoteDataSource);
    //act
    var result = await ifBallRepository.listUpFromInfluencePower(
        listUpReqDto: searchCondition);
    //assert
