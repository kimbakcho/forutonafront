...
void main() {
  ...
  setUp(() {
    navBtnMediator = TopNavBtnMediatorImpl(currentTopNavRouter: CodeState.H001CODE);
  });
  ...
  test('메디에이터 NavOpen 상태 테스트', () async {
    //arrange
    MockTopNavExpendComponent mockTopNavExpendComponent =
        MockTopNavExpendComponent();
    when(mockTopNavExpendComponent.getTopNavRouterType())
        .thenAnswer((realInvocation) => CodeState.H001CODE);
  ...
  test('메디에이터 NavClose 상태 테스트', () async {
    ...
    navBtnMediator.topNavExpendGroupViewModel = TopNavExpendGroupViewModel(
        codeStateExpendWidgetMap: Map(), topNavBtnMediator: navBtnMediator);

    navBtnMediator.currentTopNavRouter = CodeState.H003CODE;
    MockTopNavExpendComponent mockTopNavExpendComponent =
        MockTopNavExpendComponent();
    when(mockTopNavExpendComponent.getTopNavRouterType())
        .thenAnswer((realInvocation) => CodeState.H003CODE);
    navBtnMediator.topNavExpendRegisterComponent(mockTopNavExpendComponent);

    MockTopNavBtnComponent topNavBtnComponent = MockTopNavBtnComponent();
    when(topNavBtnComponent.getTopNavRouterType())
        .thenAnswer((realInvocation) => CodeState.H003CODE);
    navBtnMediator.topNavBtnRegisterComponent(topNavBtnComponent);
